...
export function extractShortcode(url: string): string | null {
  if (!url) return null;
  const match =
    url.match(/instagram\.com\/(?:[^/?#]+\/)?(?:reel|reels|p|tv|share)\/([a-zA-Z0-9_-]+)/i) ||
    ...
    for (const appId of NATIVE_APP_IDS) {
      try {
        ...
        const res = await fetch(apiUrl, {
          headers: {
            'User-Agent': NATIVE_USER_AGENTS[0],
            'X-IG-App-ID': appId,
...
export async function fetchNativeEmbedHtml(shortcode: string): Promise<string | null> {
  ...
  for (const targetUrl of targetUrls) {
    for (const userAgent of NATIVE_USER_AGENTS) {
      try {
...
export async function extractInstagramMedia(url: string): Promise<CustomInstagramMediaResult> {
  ...
  // Tier 4: Fail-Safe Native Fallback Result
  return {
    shortcode,
    mediaUrl: '',
    caption: '',
